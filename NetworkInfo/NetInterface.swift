 ...
 */
open class NetInterface : CustomStringConvertible, CustomDebugStringConvertible {
     ...
     */
    open static func allInterfaces() -> [NetInterface] {
        ...
        if getifaddrs(&ifaddrsPtr) == 0 {
            ...
            while ifaddrPtr != nil {
                ...
                if addr?.sa_family == UInt8(AF_INET) || addr?.sa_family == UInt8(AF_INET6) {
                    interfaces.append(NetInterface(data: (ifaddrPtr?.pointee)!))
                }
     ...
     */
    open static func createTestDummy(_ name:String, family:Family, address:String, multicastSupported:Bool, broadcastAddress:String?) -> NetInterface
    {
        return NetInterface(name: name, family: family, address: address, netmask: nil, running: true, up: true, loopback: false, multicastSupported: multicastSupported, broadcastAddress: broadcastAddress)
    }
    ...
    convenience init(data:ifaddrs) {
        ...
        self.init(name: String(cString: data.ifa_name),
                  family: NetInterface.extractFamily(data),
                  address: NetInterface.extractAddress(data.ifa_addr.pointee),
                  netmask: NetInterface.extractAddress(data.ifa_netmask.pointee),
                  ...
                  broadcastAddress: ((broadcastValid && data.ifa_dstaddr != nil) ? NetInterface.extractAddress(data.ifa_dstaddr.pointee) : nil))
    }
