...
-(void)coblue_verification:(CBCharacteristic *)cbchar{
    if(COBLUE_ENABLE_VERIFICATION&&!done_verify){
        [self.peri_coblue writeValue:[[NSString stringWithUTF8String:COBLUE_VERIFY_KEY]dataUsingEncoding:NSUTF8StringEncoding] forCharacteristic:cbchar type:CBCharacteristicWriteWithResponse];
        done_verify = 1;
    }
}
...
-(void)charc_terminal_write:(void*)data length:(uint32_t)length{
    writeop_done = 0;
    [self.peri_coblue writeValue:[NSData dataWithBytes:data length:length] forCharacteristic:self.charc_terminal type:CBCharacteristicWriteWithResponse];
    while(!writeop_done){
...
-(void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI{
    ...
    if([advertisementData objectForKey:@"kCBAdvDataLocalName"]&&[[advertisementData objectForKey:@"kCBAdvDataLocalName"] isEqualToString:[NSString stringWithUTF8String:COBLUE_DEVICE_NAME]]){
...
-(void)centralManager:(CBCentralManager *)central didDisconnectPeripheral:(CBPeripheral *)peripheral error:(NSError *)error{
    ...
    printf("Please restart app\n");
    //[[ViewController singleton] terminal_quit];
    coblue.peri_coblue = nil;
    coblue.service_coblue = nil;
    coblue.charc_terminal = nil;
    coblue.charc_filetransfer = nil;
    coblue = nil;
}
...
int ableToUsing(){
    if(!coblue)
        goto failed;
    if(!coblue.peri_coblue)
        goto failed;
    if(! coblue.service_coblue)
        goto failed;
    if(!coblue.charc_terminal)
        goto failed;
    if(!coblue.charc_filetransfer)
        goto failed;
    return 1;
    
failed:
    printf("coBlue is not able to using\n");
    return 0;
}

void clearForConnect(){
    if(coblue){
        coblue_quit();
        coblue.peri_coblue = nil;
        coblue.service_coblue = nil;
...
void alloccoblue_animation_thread(int i){
    //[[ViewController singleton] coBlue_updateConnectStep:i];
}

int cobule_connecting = 0;
void alloccoblue_thread(){
    
    cobule_connecting = 1;
    clearForConnect();
    pthread_setname_np("alloccoblue_thread");
    ...
    while(power_status<0){}
    
    if(power_status!=CBCentralManagerStatePoweredOn){
        printf("bluetooth device unavailable, Please turn on your phone's Bluetooth and restart this app\n");
        goto error;
    }
    
    printf("scanning device (name:%s)...\n",COBLUE_DEVICE_NAME);
    
    ASYNC_THREAD(alloccoblue_animation_thread,0);
    while(!coblue.peri_coblue){
        
    }
    //printf("coblue Peripheral found!\n");
    printf("Init Step (1/4)\n");
    ASYNC_THREAD(alloccoblue_animation_thread,1);
    while(!coblue.service_coblue){
        
    }
    //printf("coblue Service found!\n");
    printf("Init Step (2/4)\n");
    ASYNC_THREAD(alloccoblue_animation_thread,2);
    while(!coblue.charc_terminal){
        
    }
    //printf("Terminal Characteristic found!\n");
    printf("Init Step (3/4)\n");
    ASYNC_THREAD(alloccoblue_animation_thread,3);
    while(!coblue.charc_filetransfer){
        
    }
    //printf("FileTransfer Characteristic found!\n");
    printf("Init Step (4/4)\nInit Cmplt\n");
    ASYNC_THREAD(alloccoblue_animation_thread,4);
    cobule_connecting = 0;
...
int alloccoblue(){
    if(!cobule_connecting){
        ASYNC_THREAD(alloccoblue_thread,NULL);
    }
    return 0;
}
